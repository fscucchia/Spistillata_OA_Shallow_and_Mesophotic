...
file_filter_in= ''
file_filter_out=''
fout2='/Home/STAR-spis-DE1/shallow-1/shallow'
workingDir1='/Home/STAR-spis-DE1/shallow-1'
include_factor_values=c('Shallow') #'Mesophotic')
...
for(d in 1:length(dist)){
	...
	gg1 = ggplot(df_mds1, aes(x=MDS1, y=MDS2, color=factor(pH),shape=factor(depth)))
...
gg1 = ggplot(pca2, aes(x=PC1, y=PC2, color=factor(pH),shape=factor(depth)))
gg1 = gg1 + geom_point(size=4,alpha=0.7,stroke=1.5)
