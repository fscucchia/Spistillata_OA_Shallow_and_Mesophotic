...
file_filter_in= ''
file_filter_out=''
fout2=      '/Home/STAR-spis-DE1/deep-shallow/deep7.6-shallow7.6' # deep7.8-shallow7.8' # deep8.2-shallow8.2'
workingDir1='/Home/STAR-spis-DE1/deep-shallow'
include_factor_values=c('7.6') #'7.8') #'8.2')
include_factor='pH'
...
compare_type[[1]]="name";		compare[[1]]="depth_Mesophotic_vs_Shallow";				compare_title[[1]]="Mesophotic_vs_Shallow"

compare_title_levels=c("Mesophotic_vs_Shallow")
compare_title_levels_groups=c(1)

relevel_column=c('depth')
relevel_factor=list('Shallow')
...
modelMatrixTest=as.data.frame(model.matrix( ~ depth,data=colData))
dds_ = DESeqDataSetFromMatrix(countData= counts1,colData= colData,design = ~ depth)
...
for(d in 1:length(dist)){
	...
	gg1 = ggplot(df_mds1, aes(x=MDS1, y=MDS2, color=factor(pH)))
...
ggplot(data, aes(PC1, PC2, color=factor(pH,levels=c('8.2','7.8','7.6')))) + 
	geom_point(size=5) + xlab(paste0("PC1: ",percentVar[1],"% variance")) + 
	ylab(paste0("PC2: ",percentVar[2],"% variance")) +
	geom_text(aes(label=name),hjust=0.0, vjust=2.0,size=4,colour='black')
dev.off()
...
gg1 = ggplot(pca2, aes(x=PC1, y=PC2, color=factor(pH)))
gg1 = gg1 + geom_point(size=4,alpha=0.7,stroke=1.5)
